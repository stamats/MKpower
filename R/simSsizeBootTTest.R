sim.ssize.boot.t.test <- function(rx, ry = NULL, mu = 0, sig.level = 0.05, power = 0.8, 
  ...
  if(type == "two.sample"){
    ...
    rowtest2 <- function(XY, n, alternative, mu, var.equal, conf.level, R, 
                        symmetric){
      boot.t.test(XY[1:n], XY[(n+1):(2*n)], alternative = alternative,
                  mu = mu, var.equal = var.equal, conf.level = conf.level, 
                  R = R, symmetric = symmetric)$boot.p.value
    }
    ...
    if(var.equal){
      METHOD <- "Bootstrap Student two-sample t-test"
    }else{
      METHOD <- "Bootstrap Welch two-sample t-test"
    }
  }
  if(type == "one.sample"){
    
    rowtest1 <- function(X, alternative, mu, conf.level, R){
      boot.t.test(X, alternative = alternative, mu = mu, 
                  conf.level = conf.level, R = R)$boot.p.value
    }
    ...
    METHOD <- "Bootstrap one-sample t-test"
  }
  if(type == "paired"){
    ns <- seq.int(from = n.min, to = n.max, by = step.size)
    empPower <- numeric(length(ns))
    ...
    METHOD <- "Bootstrap paired t-test"
  }
