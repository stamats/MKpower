sim.power.wilcox.test <- function(rx, ry = NULL, mu = 0, sig.level = 0.05, power = 0.8, 
  ...
  if(type == "two.sample"){
    ...
    for(i in seq_len(length(ns))){
      data.x <- matrix(rx(ns[i]*iter), ncol = iter)
      data.y <- matrix(ry(ns[i]*iter), ncol = iter)
      res <- col_wilcoxon_twosample(data.x, data.y, 
                                    alternative = alternative, mu = 0,
  ...
  if(type == "one.sample"){
    ...
    for(i in seq_len(length(ns))){
      data.x <- matrix(rx(ns[i]*iter), ncol = iter)
      res <- col_wilcoxon_onesample(data.x, alternative = alternative, mu = mu,
                                    exact = NA, correct = TRUE)[,"pvalue"]
      empPower[i] <- sum(res < sig.level)/iter
      if(empPower[i] > power) if(BREAK) break
    }
  ...
  if(type == "paired"){
    ...
    empPower <- numeric(length(ns))
    for(i in seq_len(length(ns))){
      data.xy <- matrix(rx(ns[i]*iter), ncol = iter)
      res <- col_wilcoxon_onesample(data.xy, 
  ...
  if(is.null(ry))
    res <- structure(list(n = ns[!is.na(empPower)], rx = body(rx), 
                          sig.level = sig.level, power = empPower[!is.na(empPower)],
