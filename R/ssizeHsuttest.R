ssize.hsu.t.test <- function (delta = 1, sd1 = 1, sd2 = 1,
  ...
  p.body <- if (strict && tside == 2)
    ...
    pt(qt(sig.level/tside, nu, lower.tail = FALSE), nu, 
       ncp = delta/sd, lower.tail = FALSE)
  })
  n0 <- power.hsu.t.test(delta = delta, sd1 = sd1, sd2 = sd2, 
                         sig.level = sig.level, power = power, 
                         alternative = alternative, strict = strict)$n
  n0 <- ceiling(n0)
  pfun <- function(n1, n2){ eval(p.body) }
  N1 <- seq.int(from = 2, to = 2*n0, by = 1)
  N2 <- N1
  res <- outer(N1, N2, pfun)
  SUM <- outer(N1, N2, "+")
  ...
  power <- power.hsu.t.test2(n = n, delta = delta, sd1 = sd1, sd2 = sd2, 
                             sig.level = sig.level, alternative = alternative,
                             strict = strict)$power

  NOTE <- "minimum total sample size (sum of group sample sizes)"
  METHOD <- "Two-sample Hsu t test sample size calculation"
